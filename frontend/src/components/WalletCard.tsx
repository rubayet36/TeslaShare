...
export function WalletCard() {
  ...
  if (!currentUser) {
    return (
      <div className="bg-slate-900/80 rounded-2xl p-6 border border-slate-800 text-white shadow-xl backdrop-blur-sm text-center space-y-3">
        <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 mx-auto">
          <Wallet className="w-6 h-6" />
        </div>
        <h3 className="font-bold text-base text-white">TeslaPay Wallet</h3>
        <p className="text-xs text-slate-400">
          Sign in or register an account to view your balance and pay for pooled rides cashless.
        </p>
      </div>
    );
  }
